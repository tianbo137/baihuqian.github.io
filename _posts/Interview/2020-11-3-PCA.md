...
$$ \vec{x} = (\vec{f}_1, \vec{f}_2, \cdots, \vec{f}_n) \cdot \left( \begin{array}{c}\langle \vec{x}, \ \vec{f}_1 \rangle\\ \langle \vec{x}, \ \vec{f}_2 \rangle \\ \vdots \\ \langle \vec{x}, \ \vec{f}_n \rangle \end{array} \right) = \sum \langle \vec{x}, \ \vec{f}_i \rangle \vec{f}_i.$$
